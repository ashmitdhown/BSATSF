...
const TransferOwnership: React.FC = () => {
  ...
  const { getUserAssets, transferERC721, getTransferFee } = useContracts();
  ...
  const handleTransfer = async () => {
    ...
    try {
      if (selectedAsset.type === 'ERC-721') {
        ...
        const tx = await transferERC721(account, recipientAddress, selectedAsset.tokenId);
        ...
        toast.error('ERC-1155 transfers not implemented yet');
        setTransferStatus('failed');
      }
    } catch (error: any) {
                        ...
                        <div className="flex-1">
                          ...
                          <div className="space-y-1 text-sm">
                            ...
                            {selectedAsset.balance && (
                              <p className="text-gray-400">Balance: <span className="text-white">{selectedAsset.balance}</span></p>
                            )}
                          </div>
                        </div>
                      </div>
