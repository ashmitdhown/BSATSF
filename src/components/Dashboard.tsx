...
import { Search, Grid, ChevronLeft, ChevronRight, Plus, Tag, XCircle } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  const { account, balanceEth, refreshBalance, loadAssets } = useWeb3();
  const {
    erc721Contract,
    erc1155Contract,
  ...
  const loadUserAssets = async () => {
    ...
    setLoading(true);
    try {
      // 1. Fetch Market Data to check if items are listed
      const listings = await getMarketplaceListings();

      // Create a map: "type-tokenId" -> Listing Details
      ...
      try {
        ...
        console.warn("Could not fetch 721s via getMyAssets", e);
      }

      // 3. Fetch My ERC-1155s
      try {
        ...
        console.warn("Could not fetch 1155s via getMyAssets", e);
      }
  ...
  const handleCancelListing = async (asset: Asset) => {
    ...
    try {
      ...
      await loadUserAssets(); // Refresh UI to remove "Listed" status
    } catch (e: any) {
  ...
  if (loading) {
    return (
      <div className="relative w-full min-h-screen bg-[#0F1419] flex items-center justify-center">
      ...
      <div className="relative z-10 flex flex-col h-full grow">
        <div className="w-full max-w-7xl mx-auto px-6 md:px-8 lg:px-12 py-10">
          ...
          <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
            <div>
              <h1 className="text-white text-4xl font-black">My Assets</h1>
            ...
            <div className="flex items-center gap-4">
              <div className="bg-[#1A1F2E] px-4 py-2 rounded-lg border border-gray-700">
                ...
                <span className="text-[#00E0FF] font-mono font-bold">{balanceEth?.slice(0, 6)} ETH</span>
              </div>
          ...
          {filteredAssets.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {displayedAssets.map((asset) => (
                <div key={asset.id} className="group flex flex-col glass-card bg-[#1A1F2E] border border-gray-800 rounded-xl overflow-hidden hover:border-[#00E0FF] transition-all duration-300">
                  <div className="relative aspect-square bg-black/40">
                    ...
                    <div className="absolute top-2 right-2 bg-black/60 backdrop-blur px-2 py-1 rounded text-xs text-white font-mono border border-white/10">
                      {asset.type}
                    </div>
                  </div>
